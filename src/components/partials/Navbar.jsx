...
export function Navbar({ profile }) {
    ...
    const itemsNav = profile ? itemsAuth : itemsNoAuth;
                                ...
                                {itemsNav.map(item => {
                                    return (
                                        <NavLink to={item.href} key={`${item.label}-standard`}
                                            ...
                                            className="text-gray-600 dark:text-gray-300 no-underline hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium"
                                        >
                                            {item.label}
                                        </NavLink>
                                    )
                        ...
                        {itemsNav.map(item => {
                            return (
                                <NavLink to={item.href} key={`${item.label}-mobile`}
                                    activeClassName="bg-gray-900 text-white"
                                    className="block text-gray-600 dark:text-gray-300 no-underline hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium"
                                >
                                    {item.label}
                                </NavLink>
                            )
